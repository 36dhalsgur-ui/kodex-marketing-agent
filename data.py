...
#   TIGER 반도체 92.8% · 반도체TOP10 65.7% · Fn K-반도체 56.9%
#   AI반도체소부장 25.1% · 후공정 13.3% · 전공정 10.7%
# 기준선 70%는 통계가 아니라 정책이다: 잘못된 대조군으로 낸 DiD는 약한 추정이
# 아니라 인과의 탈을 쓴 틀린 숫자이므로, 확실히 같은 자산일 때만 잰다.
# 이 기준을 못 넘는 후보뿐이면 대조군을 비워 두고 DiD를 수행하지 않는다 —
# 억지로 잰 숫자보다 '측정 못 함'이 낫다. 대가도 안다: 65~69%대의 동종
# 대형주 상품(반도체TOP10 66% 등, 집중형이라 겹침 천장이 낮은 것)까지
# 잘리고, 분산형 테마(바이오, 최고 37%)는 대조군이 아예 없다.
