...
with tab_trend:
    ...
    with st.expander("지표 정의 · 진단 기준 자세히 보기"):
        st.markdown(
            """
##### 지표 정의
...
| 지표 | 정의 | 출처 |
|------|------|------|
| **외국인·기관 4주 순매수** | 최근 4주(분석 주차 포함) 해당 테마 종목에 대한 외국인+기관 순매수 금액 합계(억원). 기관 집계에서 증권사 유동성공급(LP·금융투자) 물량 제외 | KRX 투자자별 거래 (현재 데모) |
| **개인 4주 순매수** | 같은 기간 개인 투자자 순매수 금액 합계(억원) | KRX 투자자별 거래 (현재 데모) |
| **가격 4주 수익률** | 테마 소속 ETF들의 주간 수익률을 최근 4주 누적한 값(%) | 시세 데이터 (현재 데모) |
| **검색량 전주 대비** | 네이버 데이터랩 주간 검색량 지수의 전주 대비 증감률(%). 테마별 매핑 키워드 기준 (예: 반도체 → 'AI반도체') | 네이버 데이터랩 (키 설정 시 실데이터) |

##### 진단 기준 — 위에서부터 순서대로 검사, 먼저 맞는 단계로 판정

| 순서 | 단계 | 조건 |
|------|------|------|
| 1 | 🔴 **확산기** | 가격 4주 수익률 > 0 **그리고** 개인 4주 순매수 > 0 |
| 2 | 🟡 **태동기** | 외국인·기관 4주 순매수 > 0 **그리고** 개인 4주 순매수 ≤ 0 |
| 3 | 🔵 **과열기** | 외국인·기관 4주 순매수 < 0 **그리고** 개인 4주 순매수 > 0 |
| 4 | ⚪ **쇠퇴기** | 외국인·기관 < 0 **그리고** 개인 ≤ 0 **그리고** 가격 ≤ 0 |
| 5 | ▫️ 관망 | 위 어디에도 해당 없음 — 단계를 억지로 붙이지 않고 판정 유보 |

**알아둘 점**
- 부호(+/−)만 보는 러프 판정입니다. 임계값·표준화 없이 단순하게 시작하고, 실데이터가 쌓이면 과거 사이클 백테스트로 정교화할 예정입니다.
- **검색량은 진단 조건에 포함되지 않습니다** — 확산기·과열기 판정을 교차 확인하는 참고 지표입니다 (예: 확산기인데 검색량이 감소 중이면 신뢰도 하향).
- 수급 부호가 주 단위로 바뀔 수 있어 라벨도 주마다 달라질 수 있습니다. 라벨보다 원값 4개를 먼저 확인하는 습관을 권장합니다.
"""
        )
    st.write("")
