        ...
        if sc.get("did") is not None:
            ...
            if score is not None:
                ...
                if z >= 1.65:
                    ...
                    vsay = ("평소보다 조금 높지만 이벤트 효과를 단정하기엔 약합니다. "
                            "평소 변동으로도 나올 수 있는 크기라, 다음 주 값까지 봐야 방향이 잡힙니다.")
                elif z > -0.5:
                    verdict, vcolor = "평소와 차이 없음", "#C7CFDF"
                    vsay = ("평소 변동 범위 안입니다. 효과가 없었다는 뜻이 아니라, "
                            "이 지표로는 있었는지 없었는지 가려지지 않는다는 뜻입니다.")
                else:
                    verdict, vcolor = "평소보다 부진", "#9DB2D9"
                    vsay = ("평소보다 오히려 낮습니다 — 집행 주의 순유입이 평소만 못했습니다. "
                            "경쟁사 이벤트가 겹쳤는지 ② 채널 모니터링에서 함께 확인해보세요.")
                ...
                st.markdown(
                    ...
                    f'<b>평소 변동폭의 {z:+.1f}배 → {score:.0f}점.</b> {vsay}</div></div>',
                    unsafe_allow_html=True,
                )
...
- 같은 +2%p라도 평소 ±0.3%p인 ETF에서는 높은 점수, ±7%p인 ETF에서는 낮은 점수입니다.
...
**주의** — 변동폭은 해당 ETF 자신의 과거 DiD로만 잽니다. 따라서 평소에도
마케팅을 자주 한 ETF는 기준이 높아 점수가 짜게 나옵니다. 마케팅을 한 적 없는
ETF들과 비교하는 절대 기준선은 별도 작업(귀무분포)이 필요합니다.
                        """
                    )
            else:
                st.markdown(
