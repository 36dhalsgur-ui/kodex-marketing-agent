...
def _did_method_html() -> str:
    ...
    steps = "".join([
        ...
        _step(5, "0~100점 — 이 DiD가 평소 변동폭을 넘었나",
              f"z = (DiD − 평소 평균) ÷ 평소 변동폭<br>점수 = 100 ÷ (1 + e^−z)",
              ...
              f'지워집니다. <b style="color:{INK};">50점 = 평소 변동폭 안</b>, 위로 갈수록 '
              "이례적으로 강한 반응입니다."),
    ])
        ...
        if sc.get("did") is not None:
            ...
            if score is not None:
                ...
                st.markdown(
                    ...
                    f'<b>평소 DiD 변동폭의 {z:+.1f}배 → {score:.0f}점.</b> {vsay}</div></div>',
                    unsafe_allow_html=True,
                )
                with st.expander("점수 해석 기준"):
                    st.markdown(
                        f"""
**점수는 "이 DiD가 이 ETF의 평소 변동폭을 얼마나 넘었나"입니다.**
절대적인 효과 크기가 아니라 **그 상품 기준의 상대 위치**입니다.

마케팅이 없던 주에도 DiD는 출렁입니다. 그 출렁임의 폭(σ)을 잣대로 삼아,
이번 값이 몇 배나 벗어났는지를 0~100으로 옮긴 값입니다.

- **50점 = 평소 변동폭 안** (기준선). 0점이 아니라 50점이 "효과 없음"입니다.
- 예를 들어 같은 +2%p라도 평소 ±0.3%p인 ETF에서는 높은 점수, ±7%p인 ETF에서는 낮은 점수입니다.

| 점수 | 평소 변동폭 대비 | 판정 | 의미 |
|---|---|---|---|
| 84점 이상 | 1.65배 이상 | 이례적으로 강함 | 평소 분포의 상위 5% |
...
with tab_report:
    ...
    if did_all:
        ...
        st.caption("**DiD가 양수면** 같은 기간 경쟁 ETF보다 자금을 더 받았다는 뜻입니다. "
                   "**점수**는 그 값이 이 ETF의 평소 변동폭에서 얼마나 벗어났는지로, "
                   "38~62점은 평소 범위라 효과로 보기 어렵고 73점을 넘어야 효과가 "
                   "있었다고 볼 만합니다.  \n"
                   "한 주 값은 0점에서 100점까지 오갈 만큼 출렁여 판정에 쓰지 않습니다 — "
                   "**집행 기간 전체 평균**으로 재고, 집행 주수가 쌓일수록 잣대가 좁아집니다"
                   "(평균의 표준오차 = 평소 변동폭 ÷ √주수). 매주 새 데이터가 더해져 "
                   "갱신됩니다.  \n"
                   "**이번 주**와 **플러스 n/m주**는 최근 흐름을 보는 참고값이고 판정에는 "
                   "쓰지 않습니다. 주차별 추이와 대조군 구성은 ③에서 봅니다.")
        st.write("")
