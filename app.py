...
with tab_trend:
    ...
    if not board_file.exists():
        ...
        def stage_rows(rows, decline_order=False):
            # 기본: RS모멘텀 내림차순 / 쇠퇴기: 외국인·연기금 매수 중(재매집 후보) 우선 + 모멘텀 순
            if decline_order:
                rows = sorted(rows, key=lambda x: (not (smart_buy(x) > 0), -(x.get("RS모멘텀") or -99)))
            else:
        ...
        st.markdown(
            ...
            f'수급(최근 13주 순매수)은 <b>판정과 별개로 자금이 실제로 어디로 움직였는지 보여주는 보조 지표</b>이며, '
            f'쇠퇴기 표에서는 <b>외국인·연기금이 매수 중인 섹터(재매집 후보)를 상단에 배치</b>하는 데 쓰입니다.</div>'
            f"{groups_html}</div>",
            unsafe_allow_html=True,
        )
...
- **수급의 고유 가치**: 가격이 아직 움직이지 않은 "조용한 매집"은 가격 지표에 존재하지 않는 정보입니다. 그래서 쇠퇴기 표에서만 **외국인·연기금이 매수 중인 섹터(재매집 후보)를 상단에 배치**하는 데 사용합니다 (정렬 기준 = 외국인+연기금 13주 순매수 합 > 0).
