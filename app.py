...
def _did_method_html() -> str:
    ...
    steps = "".join([
        ...
        _step(2, "Δ처치 — 집행 주가 직전 8주 평균보다 얼마나 높았나",
              f"Δ처치 = 집행 주 강도 − 직전 {D.BASELINE_WEEKS}주 평균",
              f"직전 {D.BASELINE_WEEKS}주 평균을 그 상품의 <b style=\"color:{INK};\">평소 수준</b>으로 삼습니다. 한 주 순유입은 기관 설정 한 건에도 크게 튀어, 평균으로 눌러야 "
              "기준이 안정됩니다."),
        ...
        _step(5, "0~100점 — 이 DiD가 평소보다 높은가",
              f"z = (집행 기간 평균 DiD − 평소 평균) ÷ 표준오차<br>"
              f"표준오차 = 평소 변동폭 ÷ √집행주수<br>점수 = 100 ÷ (1 + e^−z)",
              "%p 값만으로는 큰지 작은지 알 수 없습니다. 그래서 이 상품의 "
              f'<b style="color:{INK};">평소 변동폭</b>과 견줍니다 — 마케팅이 없던 주에도 '
              f'DiD는 출렁이므로, 그 폭을 넘어야 효과라고 볼 수 있습니다.<br>'
              f'변동폭은 <b style="color:{INK};">이벤트 이전</b> 최대 {D.ZSCORE_WINDOW}주의 '
              "DiD로 잽니다. 집행 중인 주를 섞으면 효과가 기준선에 들어가 스스로 "
              f'지워집니다. <b style="color:{INK};">50점 = 평소와 같음</b>, 위로 갈수록 '
              "이례적으로 강한 반응입니다."),
    ])
    ...
    return (
        ...
        + f'<div style="margin-top:12px;padding-top:10px;border-top:1px solid {LINE};'
          ...
          f'기초시장</b>의 경쟁 ETF 중에서 <b style="color:{INK};">실제 구성종목이 비슷한 '
          f'것만</b> 남깁니다 — 이름 키워드가 아니라 공시 구성종목의 비중 겹침이 '
          f'{D.OVERLAP_MIN:.0f}% 이상이어야 합니다. 같은 ‘반도체’라도 대형주 ETF와 장비 '
          f'소부장 ETF는 겹침이 10%대라 대조군이 될 수 없습니다. 해외 자산은 구성종목이 '
          f'공시 API에 없어 일별 수익률 상관 {D.CORR_MIN:.2f} 이상으로 대신하고, '
          f'커버드콜 같은 전략 상품은 공시 기초지수의 기초자산 일치로 판정합니다. '
          f'커버드콜·채권혼합·환헤지(H) 여부는 처치군과 같아야 합니다. 개수 제한은 '
          f'없습니다 — 적합한 후보가 적으면 적은 대로 쓰고, 합산은 순자산 가중평균이라 '
          f'소형 종목의 출렁임이 눌립니다.</div>'
...
with tab_did:
    ...
    # 신규 상장 종목은 Δ를 낼 직전 주가 없어 주차=None 한 줄로 들어온다. 걸러내지
    # 않으면 아래 대체 주차가 None이 되고, 사유 문구의 '수집 구간'도 어긋난다.
    weeks_avail = [w for w in dict.fromkeys(netbuy_df["주차"]) if isinstance(w, str)]
    ...
    if len(_diag):
        ...
        for _, r in _diag.iterrows():
            ...
            _rows += (
                ...
                + '<td class="num" style="padding:6px 10px;font-size:0.82rem;">'
                + ("—" if pd.isna(r["평행오차"]) else f'{r["평행오차"]:.2f}') + '</td>'
    ...
    with d2:
        ...
        if sc.get("did") is not None:
            ...
            if score is not None:
                ...
                # 문장은 둘로 나눈다: 앞은 이 숫자가 무엇인지, 뒤는 그래서 어떻게 볼지.
                # 판정만 던지면 '그래서 뭘 해야 하나'가 남는다.
                z = sc["z"]
                if z >= 1.65:
                    verdict, vcolor = "이례적으로 강함", "#2E9E62"
                    vsay = ("평소 분포의 상위 5% 수준입니다 — 이벤트 효과가 뚜렷합니다. "
                            "같은 유형의 이벤트를 확대 검토할 근거가 됩니다.")
                elif z >= 1.0:
                    verdict, vcolor = "평소보다 강함", "#7FE0A7"
                    vsay = ("이벤트 효과가 있었다고 볼 만합니다. 다만 한 주 값이라, "
                            "집행이 이어지는 동안 추이를 더 봐야 합니다.")
                elif z >= 0.5:
                    verdict, vcolor = "다소 강함", "#B8E6C8"
                    vsay = ("평소 변동으로도 나올 수 있는 크기라, 이벤트 효과 여부에 대한 "
                            "판단을 유보합니다.")
                elif z > -0.5:
                    verdict, vcolor = "평소와 차이 없음", "#C7CFDF"
                    vsay = ("평소 변동 범위 안입니다. 이 지표로는 이벤트 효과 여부를 "
                            "판정하기 어렵습니다.")
                else:
                    verdict, vcolor = "평소보다 부진", "#9DB2D9"
                    vsay = ("평소보다 오히려 낮습니다 — 집행 주의 순유입이 평소만 못했습니다. "
                            "마케팅 방향성을 점검할 필요가 있습니다.")
                ...
                st.markdown(
                    ...
                    # 같은 내용이 화면에 세 번 나온다.
                    # 문장 순서: 어떻게 나온 점수인가 → 무슨 뜻인가 → 다음에 볼 것.
                    f'<div class="did-result-note">'
                    # 분모는 평균의 표준오차다. 예전 문구('평소 변동폭의 n배')는 판정을
                    # 한 주 → 집행 기간 평균으로 바꾸면서 계산과 어긋났다.
                    f'<b>평소보다 {abs(z):.1f}배만큼 {"높음" if z >= 0 else "낮음"} '
                    f'→ {score:.0f}점.</b> {vsay}</div></div>',
                    unsafe_allow_html=True,
                )
                with st.expander("점수 해석 기준"):
                    st.markdown(
                        f"""
**점수는 "집행 기간 평균 DiD가 이 ETF의 평소보다 높은가"입니다.**
절대적인 효과 크기가 아니라 **그 상품 기준의 상대 위치**입니다.

마케팅이 없던 주에도 DiD는 출렁입니다. 그 출렁임의 폭을 잣대로 삼되,
집행 주수가 쌓이면 평균이 안정되므로 잣대도 그만큼 좁힙니다
(표준오차 = 평소 변동폭 ÷ √집행주수).

- **50점 = 평소와 같음** (기준선). 0점이 아니라 50점이 "효과 없음"입니다.
- 예를 들어 같은 +2%p라도 평소 ±0.3%p인 ETF에서는 높은 점수, ±7%p인 ETF에서는 낮은 점수입니다.

| 점수 | 평소 대비 | 판정 | 의미 |
|---|---|---|---|
| 84점 이상 | 표준오차의 1.65배 이상 | 이례적으로 강함 | 평소 분포의 상위 5% |
| 73~84점 | 1.0배 이상 | 평소보다 강함 | 효과가 있었다고 볼 만함 |
| 62~73점 | 0.5배 이상 | 다소 강함 | 방향은 긍정, 단정은 이름 |
| 38~62점 | ±0.5배 이내 | 평소와 차이 없음 | 판별 불가 |
| 38점 미만 | −0.5배 이하 | 평소보다 부진 | 평소만 못함 |

**주의** — 두 기간을 헷갈리지 마세요.
`직전 {D.BASELINE_WEEKS}주 평균`은 DiD 값 자체를 구할 때 쓰는 **평소 수준**이고,
`이벤트 이전 {D.ZSCORE_WINDOW}주`는 그 DiD가 큰지 재는 **평소 변동폭**입니다.
앞은 유입강도의 평균, 뒤는 DiD 값들의 표준편차로 서로 다른 층입니다.
                        """
                    )
            else:
                st.markdown(
...
with tab_report:
    ...
    # 신규 ETF 출시 후보 (라인업 공백 1순위) — B가 "있는 상품을 밀까"라면
    # 여기는 "없는 상품을 만들까". 공백은 KODEX 미보유 테마라 B와 겹치지 않는다.
    ...
    def _gap_detail(g: dict, judged: dict | None = None) -> dict:
        ...
        # 차별화 각도 — 판정 하나로만 갈리면 후보끼리 같은 문장이 나온다(실측:
        # 5건 중 4건이 두 문장으로 수렴). 경쟁이 무엇에 쏠려 있는지(_dom)와
        # 검색 수요(_srch)를 함께 보고 '무엇으로 갈라설지'를 정한다.
        # 경쟁이 몰린 유형의 반대편이 곧 비어 있는 칸이다.
        _open = {"액티브": "지수추종", "지수추종": "액티브·커버드콜",
                 "커버드콜": "지수추종·액티브",
                 "집중(TOP)": "분산형"}.get(_dom, "다른 운용 방식")

        def _ro(w):
            """(으)로 — 받침이 없거나 ㄹ이면 '로'. '커버드콜으로'가 나오던 것을 막는다."""
        ...
        if j.get("판정") == "차별화 필요" and _share:
            ...
            _angle = f'경쟁이 {_dom}에 몰려 있어 {_open}{_ro(_open)} 빈 칸을 노릴 수 있습니다. '
        elif _all_same:
            _angle = (f'{g["경쟁사수"]}종이 모두 {_dom}이라 {_open}{_ro(_open)} 다른 수요를 '
                      f'가져올 수 있습니다. ')
        else:
            _angle = f'경쟁이 {_dom} 중심이라 {_open}{_ro(_open)} 빈틈을 노릴 수 있습니다. '
        # 검색 수요가 뚜렷하면 국면과 무관하게 소재 가치가 있다 — 그 사실을 덧붙인다
        if _srch is not None and _srch >= 20:
            _angle += f'검색이 {_srch:+.0f}%로 뜨고 있어 소재 준비는 지금 해둘 만합니다. '
        return {
            ...
            "타이밍설명": j.get("근거") or (
                "국면이 고점·쇠퇴 구간이라 지금 준비를 시작해도 늦습니다"
    ...
    with b1:
        ...
        if emerging_all:
            _VC = {"착수": ("#B0801F", "#FDF6E7"), "선점 검토": ("#2C63B5", "#EAF0FD"),
                   ...
                   "규모 미달": (COOL, "#EAF0FD"), "확인 필요": (RED, "#FDECEB"),
                   "상품 없음": (FAINT, "#F7F9FC")}
            _rows = ""
            for e in emerging_all:
                ...
                _rows += (
                    ...
                    f'<span style="font-size:0.72rem;color:{FAINT};white-space:nowrap;">'
                    f'{e["kodex"] or "상품 없음"}'
                    + (f' · {e["순자산억"]:,.0f}억' if e.get("순자산억") else "")
                    + '</span></div>'
                    ...
                    criteria_list(
                        ...
                        [
                            ...
                            ("국면이 과열기·쇠퇴기가 아닐 것",
                             ...
                             "테마는 상장 시점에 수요가 빠진 뒤일 가능성이 커, 규모가 크더라도 "
                             "후보에서 뺍니다. 반대로 태동기는 관심이 다시 붙기 시작한 구간이라 "
                             "몇 달 뒤 상장 시점과 맞물립니다 — 같은 판정 안에서 먼저 놓습니다."),
                        ],
    ...
    if did_all:
        ...
        # 줄은 주제로 가른다 — 앞줄은 DiD, 뒷줄은 점수. 판정 방식을 뒤에 붙이면
        # 짧은 문장 하나가 매달려 남는다.
        st.caption("DiD가 양수면 같은 기간 경쟁 ETF보다 자금이 더 들어왔다는 뜻입니다. "
                   "한 주 값은 크게 출렁여 집행 기간 평균으로 판정합니다.  \n"
                   "점수는 그 값이 이 ETF의 평소보다 높은지 낮은지를 0~100으로 옮긴 "
                   "값으로, 50점이 보통이고 73점보다 높아야 이벤트 효과가 있었다고 "
                   "볼 만합니다.")
        st.write("")
